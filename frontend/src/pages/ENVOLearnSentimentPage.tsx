...
import { Download, RefreshCw, TrendingUp, Users, MessageCircle, Star, BookOpen } from 'lucide-react';
...
interface AnalyticsData {
  ...
  topic_averages: TopicData[];
  topic_searches: TopicSearchData[];
  positive_percentage: number;
  negative_percentage: number;
  neutral_percentage: number;
  total_sentiment_reviews: number;
}

const ENVOLearnSentimentPage: React.FC = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      setError(null);
      const response = await fetch('/api/summary-feedback/analytics');
  ...
  const handleDownloadCSV = () => {
    ...
    // Create CSV content
    const headers = ['Date', 'Topic', 'Rating', 'Sentiment', 'Sentiment Score', 'Comment'];
    const csvContent = [
      headers.join(','),
    ...
    a.href = url;
    a.download = `envo-learn-feedback-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const topicChartData = analytics ? analytics.topic_averages.slice(0, 10).map(topic => ({
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">
              ENVO Learn Sentiment Analysis
            </h1>
            <p className="text-gray-400">
              Analyze user sentiment and feedback for educational content
            </p>
          </div>
        ...
        {analytics && (
          <>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              ...
              <div className="bg-dark-800 p-6 rounded-lg border border-dark-700">
                ...
                <p className="text-gray-400 text-sm mb-4">
                  Based on {analytics.total_sentiment_reviews} reviews with sentiment analysis 
                  (out of {analytics.total_feedback} total reviews)
                </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              {/* Topic Averages */}
              <div className="bg-dark-800 p-6 rounded-lg border border-dark-700">
                <h2 className="text-xl font-bold mb-4 text-white">Topic Averages</h2>
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {analytics.topic_averages.length > 0 ? (
                    analytics.topic_averages.map((topic, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-dark-700 rounded-lg">
                        <div className="flex-1">
                          ...
                          <div className="text-gray-400 text-sm">{topic.count} reviews</div>
                        </div>
                        <div className="text-right">
                          <div className="text-neon-yellow font-bold">{topic.avg.toFixed(1)}</div>
              ...
              <div className="bg-dark-800 p-6 rounded-lg border border-dark-700">
                ...
                <div className="space-y-3 max-h-64 overflow-y-auto">
                  {analytics.topic_searches.length > 0 ? (
                    analytics.topic_searches.slice(0, 10).map((topic, index) => (
                      <div key={index} className="flex justify-between items-center p-3 bg-dark-700 rounded-lg">
                        <div className="text-white font-medium">{topic.topic}</div>
                        <div className="text-neon-blue font-bold">{topic.count}</div>
                      </div>
                    ))
                  ) : (
                    <div className="text-gray-400 text-center py-8">No search data available</div>
                  )}
                </div>
              </div>
            </div>
                    ...
                    <div key={index} className="border-b border-dark-600 pb-4 last:border-b-0">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-white font-semibold">{feedback.topic}</span>
...
export default ENVOLearnSentimentPage;
